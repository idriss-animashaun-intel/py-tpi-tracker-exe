...
def slice_csv_creator(old_TP,new_TP,newnew_TP,site,wfr_coord, do_newnew):
    # This function pulls together all our slice info
    home = expanduser("~");
    header_path = os.getcwd();
    
    sql_orig = resource_path("sql_files\\slice_pull.txt");
    sql_new = header_path+"\\generated_files\\slice_pull.txt";
    slice_raw_sql = header_path+"\\generated_files\\slice_raw_pull.csv";

    slice_sql_pull(old_TP,new_TP,newnew_TP,sql_orig,sql_new,slice_raw_sql,site,wfr_coord, do_newnew);


def slice_sql_pull(old_TP,new_TP,newnew_TP,sql_orig,sql_new,slice_raw_sql,site,wfr_coord, do_newnew):    
